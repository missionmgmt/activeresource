...
module ActiveResource
  ...
  class Connection
    ...
    def get(path, headers = {})
      with_auth { request(:get, path, build_request_headers(headers, :get, site.merge(path))) }
    end

    # Executes a DELETE request (see HTTP protocol documentation if unfamiliar).
    # Used to delete resources.
    def delete(path, headers = {})
      with_auth { request(:delete, path, build_request_headers(headers, :delete, site.merge(path))) }
    end

    # Executes a PATCH request (see HTTP protocol documentation if unfamiliar).
    # Used to update resources.
    def patch(path, body = '', headers = {})
      with_auth { request(:patch, path, body.to_s, build_request_headers(headers, :patch, site.merge(path))) }
    end

    # Executes a PUT request (see HTTP protocol documentation if unfamiliar).
    # Used to update resources.
    def put(path, body = '', headers = {})
      with_auth { request(:put, path, body.to_s, build_request_headers(headers, :put, site.merge(path))) }
    end

    # Executes a POST request.
    # Used to create new resources.
    def post(path, body = '', headers = {})
      with_auth { request(:post, path, body.to_s, build_request_headers(headers, :post, site.merge(path))) }
    end
    ...
    def client_nonce
      Digest::MD5.hexdigest(format('%x', (Time.now.to_i + rand(65_535))))
    end
    ...
    def auth_attributes_for(uri, request_digest, params)
      ...
      auth_attrs << %(opaque="#{params['opaque']}") unless params['opaque'].blank?
      auth_attrs.join(', ')
    end
    ...
    def legitimize_auth_type(auth_type)
      ...
      auth_type.in?(%i[basic digest]) ? auth_type : :basic
    end
  end
end
